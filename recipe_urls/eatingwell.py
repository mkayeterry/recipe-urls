...
class EatingWellScraper(AbstractScraper):
    ...
    def scrape(self) -> List[str]:
        ...
        if not href_links:
            print(f"[eatingwell.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        if not unique_links_set:
            raise ValueError("[eatingwell.py] No recipe links matched the defined pattern for EatingWell.")
