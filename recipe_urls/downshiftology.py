...
class DownshiftologyScraper(AbstractScraper):
    @classmethod
    def host(cls):
        return "downshiftology"

    def scrape(self) -> List[str]:
        href_links = [a["href"] for a in self.soup.find_all("a", href=True)]

        if not href_links:
            print(f"[downshiftology.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        # Site-specific regex for Downshiftology
        ...
        if not unique_links_set:
            raise ValueError("[downshiftology.py] No recipe links matched the defined pattern for Downshiftology.")
