...
class CookieAndKateScraper(AbstractScraper):
    ...
    def scrape(self) -> List[str]:
        ...
        if not href_links:
            print(f"[cookieandkate.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        # Site-specific regex for CookieAndKate
        recipe_pattern = re.compile(r'https:\/\/cookieandkate\.com/[\w-]+-[\w-]+/')
        ...
        if not unique_links_set:
            raise ValueError("[cookieandkate.py] No recipe links matched the defined pattern for CookieAndKate.")
