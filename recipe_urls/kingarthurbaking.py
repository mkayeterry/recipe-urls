...
class KingArthurBakingScraper(AbstractScraper):
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        # Site-specific regex for KingArthurBaking
        recipe_pattern = re.compile(r'/recipes/[\w-]+-[\w-]+-recipe')

        # Use a set to deduplicate the links while filtering href links for recipe-specific ones
        unique_links_set = set(f'https://www.kingarthurbaking.com{link}' for link in href_links if recipe_pattern.search(link))
