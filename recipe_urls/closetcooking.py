...
class ClosetCookingScraper(AbstractScraper):
    @classmethod
    def host(cls):
        return "closetcooking"

    def scrape(self) -> List[str]:
        href_links = [a["href"] for a in self.soup.find_all("a", href=True)]

        if not href_links:
            print(f"[closetcooking.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        # Site-specific regex for ClosetCooking
        ...
        if not unique_links_set:
            raise ValueError("[closetcooking.py] No recipe links matched the defined pattern for ClosetCooking.")
