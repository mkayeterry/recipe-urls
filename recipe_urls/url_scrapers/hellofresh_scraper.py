...
class HelloFreshScraper(BaseScraper):
    def scrape(self) -> List[str]:
        ...
        if not href_links:
            print(f"[hellofresh_scraper.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_recipe_links(self, href_links: List[str]) -> List[str]:
        print('[hellofresh_scraper.py] Filtering general href links using specififed regex pattern...')
