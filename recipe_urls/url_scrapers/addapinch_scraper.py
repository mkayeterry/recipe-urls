...
class AddAPinchScraper(BaseScraper):
    def scrape(self) -> List[str]:
        ...
        if not href_links:
            print(f"[addapinch_scraper.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_recipe_links(self, href_links: List[str]) -> List[str]:
        print('[addapinch_scraper.py] Filtering general href links using specififed regex pattern...')

        # Site-specific regex for AddAPinch
        recipe_pattern = re.compile(r'https://addapinch\.com/.*-recipe/')
        ...
        if not unique_links_set:
            raise ValueError("[addapinch_scraper.py] No recipe links matched the defined pattern for AddAPinch.")
