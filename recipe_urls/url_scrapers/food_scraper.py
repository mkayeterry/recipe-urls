...
class FoodScraper(BaseScraper):
    def scrape(self) -> List[str]:
        ...
        if not href_links:
            print(f"[food_scraper.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_recipe_links(self, href_links: List[str]) -> List[str]:
        print('[food_scraper.py] Filtering general href links using specififed regex pattern...')
        ...
        if not recipe_links:
            raise ValueError("[food_scraper.py] No recipe links matched the defined pattern for Food.com.")
