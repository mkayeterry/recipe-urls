...
class DavidLebovitzScraper(AbstractScraper):
    @classmethod
    def host(cls):
        return "davidlebovitz"

    def scrape(self) -> List[str]:
        href_links = [a["href"] for a in self.soup.find_all("a", href=True)]

        if not href_links:
            print(f"[davidlebovitz.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        if not unique_links_set:
            raise ValueError("[davidlebovitz.py] No recipe links matched the defined pattern for DavidLebovitz.")
