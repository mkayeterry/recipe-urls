...
class BonAppetitScraper(AbstractScraper):
    ...
    def scrape(self) -> List[str]:
        ...
        if not href_links:
            print(f"[bonappetit.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        # Site-specific regex for BonAppetit
        recipe_pattern = re.compile(r'/recipe/([\w-]+)/?$')

        # Use a set to deduplicate the links while filtering href links for recipe-specific ones
        unique_links_set = set("https://www.bonappetit.com{}".format(link) for link in href_links if recipe_pattern.search(link))

        # Raise an error if no recipe links are found
        if not unique_links_set:
            raise ValueError("[bonappetit.py] No recipe links matched the defined pattern for BonAppetit.")

        else:
            print(f"[bonappetit.py] {len(unique_links_set)} recipe links found for {self.base_url}.")
