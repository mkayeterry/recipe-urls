...
class BakingSenseScraper(AbstractScraper):
    ...
    def scrape(self) -> List[str]:
        ...
        if not href_links:
            print(f"[bakingsense.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        if not unique_links_set:
            raise ValueError("[bakingsense.py] No recipe links matched the defined pattern for BakingSense.")

        else:
            print(f"[bakingsense.py] {len(unique_links_set)} recipe links found for {self.base_url}.")
