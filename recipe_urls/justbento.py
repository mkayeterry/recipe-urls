...
class JustBentoScraper(AbstractScraper):
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        # Site-specific regex for JustBento
        recipe_pattern = re.compile(r'/handbook/[\w-]+/[\w-]+-[\w-]+')

        # Use a set to deduplicate the links while filtering href links for recipe-specific ones
        unique_links_set = set(f'https://justbento.com{link}' for link in href_links if recipe_pattern.search(link) and not any(re.search(pattern, link) for pattern in unwanted_patterns))
