...
class CopyKatScraper(AbstractScraper):
    ...
    def scrape(self) -> List[str]:
        ...
        if not href_links:
            print(f"[copykat.py] Warning: href_links is empty for {self.base_url}")
    ...
    def filter_links(self, href_links: List[str]) -> List[str]:
        ...
        # Site-specific regex for CopyKat
        recipe_pattern = re.compile(r'https://copykat\.com/[\w-]+/$')
        ...
        if not unique_links_set:
            raise ValueError("[copykat.py] No recipe links matched the defined pattern for CopyKat.")
